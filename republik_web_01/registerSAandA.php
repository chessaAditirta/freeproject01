<?php
	...
	if ((empty($username))) {
		 ...
		 die(json_encode($response));
			...
			$response = new usr();
			$response->success = 0;
			$response->message = "Kolom Role User tidak boleh kosong";
			die(json_encode($response));
	} else if ((empty($gender))) {
			$response = new usr();
			$response->success = 0;
		 ...
		 if (!empty($username) && $password == $confirm_password){
		 	$num_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM users WHERE username='".$username."' NOT IN degree_of_user='R'"));
